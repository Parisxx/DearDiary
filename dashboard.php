...
function generateCalendar() {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
        ...
        let moodColor = moodData[`${currentYear}-${String(currentMonth).padStart(2, '0')}-${String(day).padStart(2, '0')}`] || '#C3C3C3'; 
        ...
        div.onclick = () => window.location.href = 'write_entry.php';    
            ...
            if (moodColor !== '#C3C3C3') {
                ...
                div.textContent = day;
                div.onclick = () => window.location.href = `entry.php?date=${currentYear}-${String(currentMonth).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
            } else {
                div.classList.add('day');
